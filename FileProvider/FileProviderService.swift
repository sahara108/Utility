...
public class FileProviderService {
  ...
  public init(cache: FileCache?) {
    self.networkProvider = FileProviderNetworkImpl()
    self.cache = cache
  }
  
  private(set) public var cache: FileCache?
...
extension FileProviderService {
  public static let service: FileProviderService = FileProviderService(cache: FileCacheImpl())
}
